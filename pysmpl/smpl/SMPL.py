...
ModelOutput = namedtuple(
    "ModelOutput", ["vertices", "joints", "joints_from_verts", "rot_mats",  "joints_wham"]
)
...
class SMPL_layer(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "v_template", to_tensor(to_np(self.smpl_data.v_template), dtype=dtype)
        )

        # The shape components
        # Shape blend shapes basis, (6890, 3, 10)
        self.register_buffer(
            "shapedirs", to_tensor(to_np(self.smpl_data.shapedirs), dtype=dtype)
        )
        ...
        self.register_buffer(
            "J_regressor_h36m", to_tensor(to_np(h36m_jregressor), dtype=dtype)
        )
        self.register_buffer(
            "J_regressor_wham", to_tensor(to_np(wham_jregressor), dtype=dtype)
        )
        ...
        parents[: (self.NUM_JOINTS + 1)] = to_tensor(
            to_np(self.smpl_data.kintree_table[0])
        ).long()
        parents[0] = -1
        ...
        self.register_buffer("parents29", parents29)

        # (6890, 23 + 1)
        self.register_buffer(
            "lbs_weights", to_tensor(to_np(self.smpl_data.weights), dtype=dtype)
        )
    ...
    def get_phi(self, betas, rotmats):
        ...
        cos = torch.cos(angle_twist)  # (b,23, 1)
        sin = torch.sin(angle_twist)  # (b, 23 ,1)
        return torch.cat([cos, sin], dim=2)

    def get_global_pose(self, pose_skeleton, rest_pose):
        rel_rest_pose = rest_pose.clone()
        rel_rest_pose[:, 1:] -= rest_pose[:, self.parents[1:]].clone()
        ...
        rel_pose_skeleton[:, 1:] = (
            rel_pose_skeleton[:, 1:] - rel_pose_skeleton[:, self.parents[1:]].clone()
        )
        rel_pose_skeleton[:, 0] = rel_rest_pose[:, 0]

        if self.train:
            globalR = batch_get_pelvis_orient(
                rel_pose_skeleton,
                rel_rest_pose,
                self.parents,
                self.children_map,
            ...
            globalR = batch_get_pelvis_orient_svd(
                rel_pose_skeleton,
                rel_rest_pose,
                self.parents,
                self.children_map,
        ...
        return roma.rotmat_to_rotvec(globalR)

    def forward(
        self, pose, betas, global_orient, pose2rot, transl=None, return_verts=True
    ):
        ...
        if transl is not None:
            ...
            vertices = (
                vertices
                - joints_from_verts_h36m[:, self.root_idx_17, :].unsqueeze(1).detach()
            )
        ...
        output = ModelOutput(
            ...
            joints=joints,
            rot_mats=rot_mats,
            joints_wham=self.J_regressor_wham @ vertices,
    ...
    def marker_ik(
        ...
        batch_size = obs_kp.shape[0]
        vertices, new_joints, rot_mats, joints_from_verts = marker_ik(
            betas,
            None,
            obs_kp,
            obs_marker,
        ...
        rot_mats = rot_mats.reshape(batch_size, 24, 3, 3)
        return roma.rotmat_to_rotvec(rot_mats)
